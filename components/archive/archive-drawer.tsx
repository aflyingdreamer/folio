...
export function ArchiveDrawer() {
  ...
  useEffect(() => {
    ...
    if (root) {
      ...
      root.style.paddingRight = open ? '360px' : '0px'
    }
    if (!open) return
  ...
  return (
    <>
      ...
      <aside
        data-folio-drawer
        className={`fixed top-0 right-0 h-full w-[360px] bg-stone-50 border-l border-stone-200 z-30 transition-transform duration-300 ease-out ${
          open ? 'translate-x-0' : 'translate-x-full'
        }`}
      >
        <div className="px-6 py-16 overflow-y-auto h-full">
